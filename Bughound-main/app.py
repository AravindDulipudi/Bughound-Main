import pymysql
from flask import Flask, send_file, flash, redirect, url_for, render_template, request, session
...
def forgot_password():
    if request.method == 'POST':
        ...
        flash('If an account with that email was found, a password reset link has been sent.', 'info')
...
def signup():
    if request.method == 'POST':
        ...
        try:
            with connection.cursor() as cursor:
                sql = "INSERT INTO employees (emp_id, name, username, password, userlevel, email_id) VALUES (%s, %s, %s, %s, %s, %s)"
                cursor.execute(sql, (emp_id, fullname, username, password, userlevel, email_id))
                connection.commit()
...
def fetch_bug_details(bug_id):
    # Connect to the database and fetch the bug details
    connection = pymysql.connect(**db_config)
    try:
        with connection.cursor() as cursor:
...
@app.route('/download_bug_pdf/<int:bug_id>')
def download_bug_pdf(bug_id):
    # Fetch the bug details
    bug_data = fetch_bug_details(bug_id)
    if bug_data:
        buffer = BytesIO()
        p = canvas.Canvas(buffer)
        p.drawString(100, 750, f"Bug ID: {bug_data['bug_id']}")
        p.drawString(100, 735, f"Program: {bug_data['program']}")
        # ... other details ...
        p.showPage()
        p.save()

        buffer.seek(0)
        return send_file(buffer, as_attachment=True, attachment_filename=f"bug_{bug_id}.pdf")
    else:
        flash('No bug found.')
        return redirect(url_for('manage_bug'))
...
def show_tables():
    ...
    try:
        with connection.cursor() as cursor:
            ...
            tables_list = [table['Tables_in_bughound'] for table in tables] # Adjust the key depending on how your database returns the table names
            print(tables_list)
            return ', '.join(tables_list) # Return the list of tables as a simple string
    finally:
        connection.close()
